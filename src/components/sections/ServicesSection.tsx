...
const ServiceItem = ({ children }: { children: React.ReactNode }) => (
  <h3
    className="text-white  font-anton text-[clamp(2rem,6vw,3.5rem)] leading-none uppercase font-medium animate-fade-in hover-scale"
...
export default function ServicesSection() {
  return (
    <section
      id="services"
      className="bg-[#6155E1] w-full h-full flex items-center relative overflow-hidden"
    >
      {/* Decorative Background Text */}
      <div
        className="absolute inset-0 leading-[1] mb-3 flex items-end  md:ml-[-27%] sm:ml-[-10%] lg:ml-[-53%] justify-center md:opacity-70 lg:opacity-70 sm:opacity-10 text-[288px] font-bold whitespace-nowrap text-transparent [-webkit-text-stroke:1px_#ffffff]"
        style={{ fontFamily: "Anton, sans-serif" }}
      >
        SER<br/>VICES
      </div>
      ...
      <div className="container mt-[-150px] mx-auto px-8 grid md:grid-cols-2 gap-16 items-center relative z-10">
        {/* Left Column */}
        <p className="text-white h-[98%] font-[Anton] text-[40px] uppercase mb-4 animate-fade-in animate-delay-1 ">
          Services
        </p>
        <div className="md:col-start-2 animate-slide-in-right">
          <p className="text-white font-[Manrope] text-[16px] font-medium text-opacity-80 max-w-md mb-12 animate-fade-in animate-delay-2">
